...
| 항목 | 점검 포인트 및 권장 조치 |
| :--- | :--- |
| **10MB 한도 오버헤드 주의 (RequestByteThreshold)** | Pub/Sub 서버의 물리 한도는 10,000,000 바이트입니다. 바이트 임계치를 정확히 10MB로 설정하면 gRPC 프레임/메타데이터 오버헤드로 인해 간헐적으로 `INVALID_ARGUMENT` 오류가 발생할 수 있습니다. 반드시 마진을 두어야 합니다. |
| **Graceful Shutdown** | 애플리케이션 종료 시 Publisher 클라이언트를 반드시 `shutdown()` 또는 버퍼 플러시(`awaitTermination`)하여 메모리에 남아있는 배치가 유실되지 않도록 해야 합니다. |
| **Ordering Key 사용 시 주의점** | `enable_message_ordering = true` 설정 시 동일한 Ordering Key를 가진 메시지는 순서대로 발행됩니다. 단, 앞선 배치가 실패하면 동일 키의 후속 메시지가 블록되므로 실패 처리에 주의해야 합니다. |
